...
contains
  ...
  subroutine write_to_laddie_output_file_grid_field( mesh, laddie, forcing, ncid, choice_output_field)
    ...
    ! ============
      ...
      case ('S_base')
        call map_from_mesh_vertices_to_xy_grid_2D( mesh, grid, C%output_dir, laddie%S_base, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, laddie%output_grid_filename, ncid, 'S_base', d_grid_vec_partial_2D)
      case ('T_amb')
      ...
      case ('S_amb')
        call map_from_mesh_vertices_to_xy_grid_2D( mesh, grid, C%output_dir, laddie%S_amb, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, laddie%output_grid_filename, ncid, 'S_amb', d_grid_vec_partial_2D)
      case ('T_freeze')
  ...
  subroutine create_laddie_output_file_grid_field( filename, ncid, choice_output_field)
    ...
    ! ============
      ...
      case ('S_base')
        call add_field_grid_dp_2D( filename, ncid, 'S_base', long_name = 'Salinity at ice/ocean interface', units = 'PSU')
      case ('T_amb')
        call add_field_grid_dp_2D( filename, ncid, 'T_amb', long_name = 'Temperature at interface with ambient ocean', units = 'deg C')
      case ('S_amb')
        call add_field_grid_dp_2D( filename, ncid, 'S_amb', long_name = 'Salinity at interface with ambient ocean', units = 'PSU')
      case ('T_freeze')
