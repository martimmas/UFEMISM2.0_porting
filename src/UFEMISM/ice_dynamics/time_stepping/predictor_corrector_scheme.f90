  ...
  subroutine run_ice_dynamics_model_pc( region, dt_max)
    ...
    do vi = region%mesh%vi1, region%mesh%vi2
      if (region%ice%mask_noice( vi)) then
        ...
        !region%ice%dHi_dt( vi)  = 0._dp
      !if (region%ice%Hi_prev(vi) < 1e-10_dp) then
      !   region%ice%dHi_dt(vi)  = 0._dp
      !end if
    end do
    ...
    iterate_pc_timestep: do while (pc_it < C%pc_nit_max)
      ...
      ! Making sure no ice vertices have non-zero thinning rates
      !do vi = region%mesh%vi1, region%mesh%vi2
      !  if (region%ice%Hi_prev(vi) < 1e-10_dp) region%ice%pc%dHi_dt_Hi_n_u_n( vi) = 0._dp
      !end do
    ...
    do vi = region%mesh%vi1, region%mesh%vi2
      if (region%ice%Hi_next(vi) < 0._dp) then
        call crash( '142 Negative ice thickness detected in PC scheme initialisation at vertex {int_01}: Hi_prev = {dp_01}', int_01 = vi, dp_01 = region%ice%Hi_prev( vi))
      end if
    end do
      ...
      do vi = region%mesh%vi1, region%mesh%vi2
        if (region%ice%Hi_next(vi) < 0._dp) then
          call crash('Hi became negatif in the PC scheme with a value of', region%ice%Hi_next(vi), 'at vertex', vi)
        end if
    #endif
