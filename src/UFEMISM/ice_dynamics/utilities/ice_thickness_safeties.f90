...
contains
  ...
  subroutine alter_ice_thickness( mesh, ice, Hi_old, Hi_new, refgeo, time, climate)
    ...
    if (C%choice_calving_law == 'threshold_thickness') then
      do vi = mesh%vi1, mesh%vi2
        if ((is_floating( Hi_eff_new( vi), ice%Hb( vi), ice%SL( vi)) .and. Hi_eff_new( vi) < C%calving_threshold_thickness_shelf) .and. (ice%mask_cf_fl( vi) .eqv. .TRUE.)) then
          Hi_new( vi) = 0._dp
        end if
      end do
    end if
    ...
    if (C%do_use_ISMIP_future_shelf_collapse_forcing) then
      ...
      case('calving')
        do vi = mesh%vi1, mesh%vi2
          if (climate%ISMIP_style%shelf_collapse_mask( vi) > 0.01_dp .and. refgeo%Hb( vi) < 0._dp .and. refgeo%Hi( vi) == 0._dp) then
            Hi_new( vi) = 0._dp
          end if
        end do
      case default
