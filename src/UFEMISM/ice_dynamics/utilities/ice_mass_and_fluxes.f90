  ...
  subroutine calc_icesheet_volume_and_area_PD( mesh, refgeo_PD, scalars)
    ...
    do vi = mesh%vi1, mesh%vi2
      ...
      if (refgeo_PD%Hi( vi) > 0._dp) then
        ...
        scalars%ice_area_PD      = scalars%ice_area_PD      + mesh%A( vi)! * 1.0E-06_dp ! [m^2]; uncomment multiplier to revert to km^2
  ...
  subroutine calc_icesheet_volume_and_area( mesh, ice, scalars)
    ...
    do vi = mesh%vi1, mesh%vi2
      ...
      if (ice%mask_grounded_ice( vi) .or. ice%mask_floating_ice( vi)) then
        ...
        scalars%ice_shelf_volume = scalars%ice_shelf_volume + max( 0._dp, (ice%Hi( vi) * mesh%A( vi) * (1._dp - ice%fraction_gr( vi))))! * 1.0E-09_dp ! [m^3]; uncomment multiplier to revert to km^3
      end if
...
! =======================
  ...
  !< Calculate all scalars in ISMIP format
    ...
    DO vi = region%mesh%vi1, region%mesh%vi2
      ...
      ! Integrated values
      ! =================
      land_ice_mass                     = land_ice_mass                     + (Hi_tot(                  vi) * region%mesh%A( vi) * ice_density)    ! kg
      mass_above_floatation             = mass_above_floatation             + MAX( 0._dp, TAF_tot(      vi) * region%mesh%A( vi) * ice_density)    ! kg
      grounded_ice_sheet_area           = grounded_ice_sheet_area           + (region%ice%fraction_gr( vi)  * region%mesh%A( vi))                  ! m2
