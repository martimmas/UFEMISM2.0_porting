...
contains
  ...
  subroutine run_bed_roughness_nudging_H_dHdt_flowline( mesh, grid_smooth, ice, target_geometry, bed_roughness)
    ...
    select case (C%choice_bed_roughness_nudging_limit_method)
      case ('Martin2011')
          ! Limit maximum bed roughness based on Martin et al. (2011) till friction angle parameterisation
      ...
      case default
        call crash('unknown choice_bed_roughness_nudging_limit_method "' // trim(C%choice_bed_roughness_nudging_limit_method) // '"')
    end select
