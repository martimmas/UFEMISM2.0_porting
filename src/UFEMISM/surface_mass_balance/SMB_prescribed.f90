module SMB_prescribed
  ...
  type, extends(atype_SMB_model) :: type_SMB_model_prescribed
    ...
    contains
      ...
      procedure, public :: deallocate_SMB_model => deallocate_SMB_model_prescribed
...
contains
  ...
  subroutine allocate_SMB_model_prescribed_abs( self, context)
    ...
    call allocate_SMB_model_prescribed( self%mesh, self)
  ...
  subroutine allocate_SMB_model_prescribed( mesh, self)

    ! In/output variables:
    type(type_mesh),                 intent(in   ) :: mesh
    type(type_SMB_model_prescribed), intent(inout) :: self
  ...
  subroutine deallocate_SMB_model_prescribed( self)

    ! In/output variables:
    class(type_SMB_model_prescribed), intent(inout) :: self

    ! Local variables:
    character(len=1024), parameter :: routine_name = 'deallocate_SMB_model_prescribed'
  ...
  end subroutine deallocate_SMB_model_prescribed
  ...
  subroutine initialise_SMB_model_prescribed_abs( self, context)
    ...
    ! Retrieve input variables from context object
    call initialise_SMB_model_prescribed( self%mesh, self%s, self%region_name())
  ...
  subroutine run_SMB_model_prescribed_abs( self, context)
    ...
    ! Retrieve input variables from context object
    call run_SMB_model_prescribed( self%mesh, self%s, self%region_name())
  ...
  subroutine run_SMB_model_prescribed( mesh, SMB, region_name)
  ...
  subroutine initialise_SMB_model_prescribed_notime( mesh, SMB, region_name)
    ...
    if (timeframe_SMB_prescribed == 1E9_dp) then
      ...
      call read_field_from_file_2D( filename_SMB_prescribed, &
        'SMB||surface_mass_balance||', mesh, C%output_dir, SMB%SMB)
    else
