module mpi_distributed_memory_grid
  ...
  interface distribute_gridded_data_from_primary
    procedure distribute_gridded_data_from_primary_logical_2D
    procedure distribute_gridded_data_from_primary_logical_3D
...
subroutine distribute_gridded_data_from_primary_logical_2D( grid, d_grid_vec_partial, d_grid)
  ...
  logical, dimension(:  ),           intent(  out) :: d_grid_vec_partial
  logical, dimension(:,:), optional, intent(in   ) :: d_grid

  ! Local variables:
  character(len=256), parameter      :: routine_name = 'distribute_gridded_data_from_primary_logical_2D'
  integer                            :: n,i,j
...
end subroutine distribute_gridded_data_from_primary_logical_2D

subroutine distribute_gridded_data_from_primary_logical_3D( grid, d_grid_vec_partial, d_grid)
  ...
  ! Local variables:
  character(len=256), parameter        :: routine_name = 'distribute_gridded_data_from_primary_logical_3D'
  integer                              :: k
  ...
  ! allocate memory
  if (par%primary) then
     allocate( d_grid_2D( size( d_grid,1), size( d_grid,2)), source = .false.)
  else
     allocate ( d_grid_2d(0,0))
  end if

  allocate( d_grid_vec_partial_2D( size( d_grid_vec_partial,1)), source = .false.)

  ! Treat each layer as a separate 2-D field
  do k = 1, size( d_grid_vec_partial,2)
    if (par%primary) d_grid_2D = d_grid( :,:,k)
    call distribute_gridded_data_from_primary_logical_2D( grid, d_grid_vec_partial_2D, d_grid_2D)
...
end subroutine distribute_gridded_data_from_primary_logical_3D
