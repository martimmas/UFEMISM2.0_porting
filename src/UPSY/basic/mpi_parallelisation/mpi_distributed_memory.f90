module mpi_distributed_memory
  ...
  interface gather_to_primary
    procedure gather_to_primary_logical_1D
    procedure gather_to_primary_logical_2D
...
! =======================================================
  ...
  subroutine gather_to_primary_logical_1D( d_partial, d_tot)
    ...
    logical , dimension(:), optional, intent(  out) :: d_tot

    ! Local variables:
    character(len=256), parameter :: routine_name = 'gather_to_primary_logical_1D'
    integer                       :: ierr,n1,i
    integer                       :: n_tot
    ...
    call MPI_GATHERV( d_partial, n1, MPI_LOGICAL, d_tot, counts, displs, MPI_LOGICAL, 0, MPI_COMM_WORLD, ierr)

    ! Finalise routine path
    call finalise_routine( routine_name)

  end subroutine gather_to_primary_logical_1D

  subroutine gather_to_primary_logical_2D( d_partial, d_tot)
    ...
    ! Local variables:
    character(len=256), parameter :: routine_name = 'gather_to_primary_logical_2D'
    ...
    do j = 1, n2
      if (par%primary) then
        call gather_to_primary_logical_1D( d_partial(:,j), d_tot( :, j))
      else
        call gather_to_primary_logical_1D( d_partial(:,j), dummy)
      end if
    end do
  ...
  end subroutine gather_to_primary_logical_2D
