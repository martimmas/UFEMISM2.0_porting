...
contains
  ...
  subroutine test_io_grid_logical_2D( test_name_parent, grid)
    ...
    call flds_reg%create_field( d1, w1, &
      grid, Arakawa_grid%a(), &
      name      = name, &
      long_name = long_name, &
      units     = units)
    ...
    call flds_reg%items(1)%p%write_to_netcdf ( filename, ncid)
    call close_netcdf_file( ncid)

    call flds_reg%create_field( d2, w2, &
      grid, Arakawa_grid%a(), &
      name      = name // '_from_file', &
      long_name = long_name, &
      units     = units)

    call flds_reg%items(2)%p%read_from_netcdf( filename, ncid)
