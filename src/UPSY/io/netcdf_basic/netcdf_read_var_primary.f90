module netcdf_read_var_primary
  ...
  interface read_var_primary
    procedure read_var_primary_logical_0D
    procedure read_var_primary_logical_1D
    procedure read_var_primary_logical_2D
    procedure read_var_primary_logical_3D
    procedure read_var_primary_logical_4D
...
subroutine read_var_primary_logical_0D(  filename, ncid, id_var, d)
  ...
  ! Local variables:
  character(len=1024), parameter :: routine_name = 'read_var_primary_logical_0D'
  ...
  ! Read the data
  if (par%primary) then
    call handle_netcdf_error( NF90_GET_VAR( ncid, id_var, d_int), &
...
end subroutine read_var_primary_logical_0D

subroutine read_var_primary_logical_1D( filename, ncid, id_var, d, start, count)
  ! Read data from a NetCDF file
  !
  ! NOTE: only the primary actually reads data! Distributing to other processes
  !       must be done afterward
  ...
  integer,                          intent(in   ) :: ncid
  integer,                          intent(in   ) :: id_var
  logical,  dimension(:),           intent(  out) :: d
  integer,  dimension(1), optional, intent(in   ) :: start, count

  ! Local variables:
  character(len=1024), parameter          :: routine_name = 'read_var_primary_logical_1D'
  character(len=1024)                     :: var_name
  integer                                 :: var_type
  integer                                 :: ndims_of_var
  ...
  integer, dimension(size(d,1))           :: d_int
  ...
  ! Read the data
  if (par%primary) then
    call handle_netcdf_error( NF90_GET_VAR( ncid, id_var, d_int, start_applied, count_applied), &
      filename = filename, dimvarname = var_name)
    d = int2logical( d_int)
  end if
...
end subroutine read_var_primary_logical_1D

subroutine read_var_primary_logical_2D( filename, ncid, id_var, d, start, count)
  ! Read data from a NetCDF file
  !
  ! NOTE: only the primary actually reads data! Distributing to other processes
  !       must be done afterward
  ...
  ! Local variables:
  character(len=1024), parameter          :: routine_name = 'read_var_primary_logical_2D'
  character(len=1024)                     :: var_name
  integer                                 :: var_type
  integer                                 :: ndims_of_var
  integer,  dimension( NF90_MAX_VAR_DIMS) :: dims_of_var
  ...
  integer, dimension(size(d,1),size(d,2)) :: d_int
...
end subroutine read_var_primary_logical_2D

subroutine read_var_primary_logical_3D( filename, ncid, id_var, d, start, count)
  ! Read data from a NetCDF file
  !
  ! NOTE: only the primary actually reads data! Distributing to other processes
  !       must be done afterward
  ...
  ! Local variables:
  character(len=1024), parameter          :: routine_name = 'read_var_primary_logical_3D'
  character(len=1024)                     :: var_name
  integer                                 :: var_type
  integer                                 :: ndims_of_var
  integer,  dimension( NF90_MAX_VAR_DIMS) :: dims_of_var
  ...
  integer, dimension(size(d,1),size(d,2),size(d,3)) :: d_int
...
end subroutine read_var_primary_logical_3D

subroutine read_var_primary_logical_4D( filename, ncid, id_var, d, start, count)
  ! Read data from a NetCDF file
  !
  ! NOTE: only the primary actually reads data! Distributing to other processes
  !       must be done afterward
  ...
  ! Local variables:
  character(len=1024), parameter          :: routine_name = 'read_var_primary_logical_4D'
  character(len=1024)                     :: var_name
  integer                                 :: var_type
  integer                                 :: ndims_of_var
  integer,  dimension( NF90_MAX_VAR_DIMS) :: dims_of_var
  ...
  integer, dimension(size(d,1),size(d,2),size(d,3),size(d,4)) :: d_int
...
end subroutine read_var_primary_logical_4D
...
! (because NetCDF doesn't support logicals)
! ======================================

elemental logical function int2logical( d_int)
  integer, intent(in) :: d_int
  int2logical = d_int == 1
